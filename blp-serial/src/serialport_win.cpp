...
void EIO_Open(uv_work_t *req)
{
  ...
  DWORD msPerByte = (data->baudRate > 0) ? ((1000 * serialBitsPerByte + data->baudRate - 1) / data->baudRate) : 1;
  if (msPerByte < 1)
  {
    msPerByte = 1;
  }
...
void EIO_List(uv_work_t *req)
{
  ...
  while (true)
  {
    ...
    if (isCom)
    {
      // TODO is com port a usb device detection
      // TODO propper error handling

/*
        PSTRING_DESCRIPTOR_NODE StringDescs = GetAllStringDescriptors(
            hHubDevice,
            portIndex,
