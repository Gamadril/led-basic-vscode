...
void ErrorCodeToString(const char *prefix, int errorCode, char *errorStr) {
  ...
  case ERROR_FILE_NOT_FOUND:
    _snprintf_s(errorStr, ERROR_STRING_SIZE, _TRUNCATE, "%s: File not found",
                prefix);
    break;
  case ERROR_INVALID_HANDLE:
    _snprintf_s(errorStr, ERROR_STRING_SIZE, _TRUNCATE, "%s: Invalid handle",
                prefix);
    break;
  case ERROR_ACCESS_DENIED:
    _snprintf_s(errorStr, ERROR_STRING_SIZE, _TRUNCATE, "%s: Access denied",
                prefix);
    break;
  case ERROR_OPERATION_ABORTED:
    _snprintf_s(errorStr, ERROR_STRING_SIZE, _TRUNCATE, "%s: Operation aborted",
                prefix);
    break;
...
void EIO_Open(uv_work_t *req) {
  ...
  HANDLE file = CreateFile(
      ...
      shareMode, // dwShareMode 0 Prevents other processes from opening if they
                 // request delete, read, or write access
      NULL, OPEN_EXISTING,
      FILE_FLAG_OVERLAPPED, // allows for reading and writing at the same time
                            // and sets the handle for asynchronous I/O
      NULL);
...
void __stdcall WriteIOCompletion(DWORD errorCode, DWORD bytesTransferred,
  ...
  if (!GetOverlappedResult((HANDLE)baton->fd, ov, &bytesWritten, TRUE)) {
    ...
    ErrorCodeToString("Writing to COM port (GetOverlappedResult)", errorCode,
                      baton->errorString);
    baton->complete = true;
    return;
  }
...
DWORD __stdcall WriteThread(LPVOID param) {
  ...
  while (!baton->complete) {
    ...
    // WriteFileEx requires calling GetLastError even upon success. Clear the
    // error beforehand.
    SetLastError(0);
    ...
    // IOCompletion routine is only called once this thread is in an alertable
    // wait state.
...
NAN_METHOD(Read) {
  ...
  if ((bytesToRead + offset) > bufferLength) {
    Nan::ThrowTypeError(
        "'bytesToRead' + 'offset' cannot be larger than the buffer's length");
    return;
  }
...
void __stdcall ReadIOCompletion(DWORD errorCode, DWORD bytesTransferred,
  ...
  if (!GetOverlappedResult((HANDLE)baton->fd, ov, &bytesTransferred, TRUE)) {
    ...
    ErrorCodeToString("Reading from COM port (GetOverlappedResult)", lastError,
                      baton->errorString);
    baton->complete = true;
    return;
  }
  ...
  if (!ReadFile((HANDLE)baton->fd, offsetPtr, baton->bytesToRead,
                &bytesTransferred, ov)) {
    errorCode = GetLastError();
...
DWORD __stdcall ReadThread(LPVOID param) {
  ...
  while (!baton->complete) {
    ...
    // ReadFileEx doesn't use overlapped's hEvent, so it is reserved for user
    // data.
    ...
    // error beforehand.
    SetLastError(0);
    // Only read 1 byte, so that the callback will be triggered once any data
    // arrives.
...
void EIO_Close(uv_work_t *req) {
  ...
  /*
    ...
    if (!GetCommState((HANDLE)data->fd, &dcb)) {
      ErrorCodeToString(
        "Close (GetCommState)", GetLastError(), data->errorString);
      return;
    }
    ...
    if (!SetCommState((HANDLE)data->fd, &dcb)) {
      ErrorCodeToString(
        "Close (SetCommState)", GetLastError(), data->errorString);
      return;
    }*/
...
void EIO_List(uv_work_t *req) {
  ...
  while (true) {
    ...
    if (isCom) {
      ...
      BOOL result = updateInfo(resultItem, &deviceInfoData, hDevInfo);
      if (!result || resultItem->vendorId != 0x16C0) {
        delete resultItem;
      } else {
...
void setNum(v8::Local<v8::Object> item, std::string key, int value) {
...
void EIO_Drain(uv_work_t *req) {
  ...
  if (!FlushFileBuffers((HANDLE)data->fd)) {
    ErrorCodeToString("Draining connection (FlushFileBuffers)", GetLastError(),
                      data->errorString);
    return;
  }
}
