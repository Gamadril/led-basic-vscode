...
#define OBJECT_ITEM_COM_NAME "comName"
...
#define OBJECT_ITEM_BCDDEVICE "bcdDevice"
...
void EIO_AfterList(uv_work_t *req)
{
  ...
  else
  {
    ...
    for (std::list<ListResultItem *>::iterator it = data->results.begin(); it != data->results.end(); ++it, i++)
    {
      ...
      Nan::Set(item, Nan::New<v8::String>(OBJECT_ITEM_COM_NAME).ToLocalChecked(), Nan::New<v8::String>((*it)->comName.c_str()).ToLocalChecked());
			...
			Nan::Set(item, Nan::New<v8::String>(OBJECT_ITEM_BCDDEVICE).ToLocalChecked(), Nan::New<v8::Number>((*it)->bcdDevice));
