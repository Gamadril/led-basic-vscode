...
SerialPort.list = function (callback) {
  SerialPortBinding.list((err, result) => {
    if (err) {
      debug('SerialPortBinding.list had an error', err);
      return this._error(err, callback);
    }
    if (callback) {
      let blpPorts = [];
      if (result) {
        blpPorts = result.filter((port) => {
          return port.vendorId && port.vendorId === 0x16c0;
        });
      }  
      callback.call(this, null, blpPorts);
    }
  });
};
